...
const NavLink = memo(({ onClick, title, children }) => (
  <button onClick={onClick} className={linkClass} title={title}>
    <span>{children}</span>
  </button>
));
  ...
  return (
    <nav aria-label="Main navigation" style={{ ...headerStyle }} className="fixed top-0 right-0 left-0 p-3 z-20 backdrop-blur-md bg-black/70 border-b border-white/5">
      <div className="container mx-auto flex items-center justify-between">
        {/* Logo — left, visible on all breakpoints */}
        <Link title="Homepage" className="flex shrink-0" to="/">
          <StaticImage
            ...
            width={50}
            height={50}
            className="rounded-full my-1 lg:my-2"
          />
        </Link>
        ...
        {/* Mobile cluster — right: visible Merch + hamburger */}
      ...
      {/* Mobile dropdown — all options except Merch */}

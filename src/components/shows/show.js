    ...
    if (d.setHours(0,0,0,0) >= today.setHours(0,0,0,0)) {
        ...
        return <div className={baseStyles}>
            ...
            <div className='flex flex-row flex-grow place-items-center justify-end'>
                ...
                <a title="Event" href={item.event} rel="noreferrer" target="_blank"><button className="lg:px-6 lg:py-4 m-2 btn-outline-primary transition duration-500 ease-in-out focus:outline-none focus:shadow-outline border border-amber-300 hover:bg-amber-200 text-amber-300 hover:text-black font-normal px-4 rounded">Event</button></a>
                {item.tickets && <a title="Tickets" href={item.tickets} rel="noreferrer" target="_blank"><button className="lg:px-6 lg:py-4 m-2 btn-outline-primary transition duration-500 ease-in-out focus:outline-none focus:shadow-outline border border-amber-300 hover:bg-amber-200 text-amber-300 hover:text-black font-normal px-4 rounded">Tickets</button></a>}
            </div>
        </div>
    }
}

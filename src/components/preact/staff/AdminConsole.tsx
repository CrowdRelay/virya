...
export default function AdminConsole() {
  ...
  async function login(event: SubmitEvent) {
    ...
    try {
      ...
      setMessage({
        tone: "error",
        text: error instanceof Error ? error.message : "Logowanie nie powiodło się",
      })
    } finally {
      setBusy(false)
    }
  ...
  if (state === "checking") return <StaffStatusCard title="Sprawdzam dostęp…" loading />
  ...
  if (state === "login") {
    return (
      <StaffLoginCard
        eyebrow="VIRYA // STAFF"
        title="Panel zespołu"
        description="Koncerty, fani, sprzedaż i komunikacja w jednym miejscu. Sesja wygasa po 12 godzinach."

...
function validateTicketForm(form: TicketForm): string | null {
  ...
  if (!/^[A-Z]{3}$/.test(currency)) return "Waluta musi mieć dokładnie 3 litery."
  if (!Number.isFinite(vat) || vat < 0 || vat > 100) return "VAT musi być w zakresie 0–100%."
  ...
  if (!Number.isInteger(holdSeconds) || holdSeconds < 2_100 || holdSeconds > 86_400) {
    return "Rezerwacja musi trwać od 2100 do 86400 sekund."
  }
  if (!Number.isFinite(salesOpenAt) || !Number.isFinite(salesCloseAt) || salesOpenAt >= salesCloseAt) {
  ...
  for (const type of form.ticketTypes) {
    ...
    const typeCapacity = type.capacity.trim() === "" ? null : Number(type.capacity)
    ...
    if (!Number.isFinite(priceGross) || priceGross < 0.01 || priceGross > 10_000_000) {
      return `Cena typu „${slug}” musi mieścić się w zakresie 0,01–10 000 000.`
    }
    if (
      typeCapacity !== null &&
      (!Number.isInteger(typeCapacity) || typeCapacity < 1 || typeCapacity > capacity)
    ) {
        ...
        <form onSubmit={save} class="grid gap-5">
          ...
          <section class="rounded-3xl border border-white/10 bg-zinc-900/70 p-5 sm:p-6">
            <div class="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h3 class="text-lg font-black text-white">Ustawienia sprzedaży</h3>
  ...
  return (
    <section class="rounded-3xl border border-white/10 bg-zinc-900/70 p-5 sm:p-6">
      ...
      <div class="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-7">
        <Metric label="Sprzedane bilety" value={String(overview.paid_tickets)} />
        <Metric
        ...
        />
        <Metric label="Opłacone zamówienia" value={String(overview.paid_orders)} />
        <Metric

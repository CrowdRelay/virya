...
const ALLOWED_ANCHOR_STATUS = new Set([
  "queued",
  "processing",
  "confirmed",
  "failed",
...
export type PublicDrawProofState =
  ...
  | {
      kind: "scheduled" | "running" | "cancelled" | "completed_pending"
      status: PublicDrawStatus
    }
  | { kind: "proof_unavailable"; status: PublicDrawStatus }
  | { kind: "ready"; status: PublicDrawStatus; proof: PublicDrawProof }

export function publicDrawApiBase() {
  const configured =
    readServerEnv(
...
function parseStatus(value: unknown): PublicDrawStatus | null {
  ...
  if (!drawSlug || !drawName || !status || !ALLOWED_STATUS.has(status))
    return null
...
export function parsePublicDrawProof(value: unknown): PublicDrawProof | null {
  ...
  if (
    !seedHash ||
    !SHA256.test(seedHash) ||
    !revealedSeed ||
    revealedSeed.length !== 64 ||
    !SHA256.test(revealedSeed)
  )
    return null
  if (
    !candidateHash ||
  ...
  const completedAt = raw.completed_at
  if (
    !nonNegativeInteger(eligibleCount, 10_000_000) ||
    !nonNegativeInteger(totalEntries, 1_000_000_000)
  )
    return null
  if (
    !nonNegativeInteger(requestedWinners, 100_000) ||
    !nonNegativeInteger(selectedWinners, 100_000)
  )
    return null
  if (selectedWinners > requestedWinners || selectedWinners > eligibleCount)
    return null
  if (typeof raw.locally_verified !== "boolean" || !validTimestamp(completedAt))
    return null
  ...
  const signerFingerprint = optionalBoundedString(
  ...
  if (
    ...
    !UUID.test(batchId) ||
    !anchorStatus ||
    !ALLOWED_ANCHOR_STATUS.has(anchorStatus)
  )
    return null
  if (
    anchor.anchor_kind !== null &&
    anchor.anchor_kind !== undefined &&
    anchorKind === null
  )
    return null
  if (
    anchor.anchor_url !== null &&
    anchor.anchor_url !== undefined &&
    anchorUrl === null
  )
    return null
  if (
    anchor.entry_id !== null &&
    anchor.entry_id !== undefined &&
    entryId === null
  )
    return null
  if (
    anchor.signer_fingerprint !== null &&
    anchor.signer_fingerprint !== undefined &&
...
export async function loadPublicDrawProofState(
  slug: string,
): Promise<PublicDrawProofState> {
  const drawRef = resolvePublicDrawProof(slug)
  let base: URL
  try {
    base = publicDrawApiBase()
  } catch {
  ...
  try {
    statusResponse = await fetch(
      new URL(
        `public/proofs/draws/${encodeURIComponent(drawRef.drawSlug)}/status`,
        base,
      ),
      {
  ...
  if (statusResponse.status === 404)
    return { kind: "not_found", drawSlug: drawRef.drawSlug }
  ...
  try {
    status = parseStatus(
      await readLimitedJson<unknown>(statusResponse, STATUS_RESPONSE_BYTES),
    )
  } catch {
    status = null
  }
  ...
  if (status.status === "draft" || status.status === "scheduled")
    return { kind: "scheduled", status }
  if (!status.proof_available) return { kind: "completed_pending", status }

  let proofResponse: Response
  try {
    proofResponse = await fetch(
      new URL(
        `public/proofs/draws/${encodeURIComponent(drawRef.drawSlug)}`,
        base,
      ),
      {
        headers: { Accept: "application/json" },
  ...
  try {
    const proof = parsePublicDrawProof(
      await readLimitedJson<unknown>(proofResponse, PROOF_RESPONSE_BYTES),
    )

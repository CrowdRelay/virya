...
export const areaTicketRewardConfigs = (): AreaTicketRewardConfig[] => {
  const raw = readServerEnv(
    ...
    import.meta.env.AREA_TICKET_REWARDS_JSON,
  )
  ...
  try {
    ...
    console.error(
      "[area-ticket-config] invalid AREA_TICKET_REWARDS_JSON",
      error,
    )
    return []
  }
...
const crowdRelayBaseUrl = () => {
  const value =
    ...
    )?.trim() || "https://signal-api.virya.music/v1/"
...
export const issueAreaTicket = async (args: {
  ...
  const response = await fetch(
    new URL("admin/admission/passes", crowdRelayBaseUrl()),
    {
      method: "POST",
      headers: {
  ...
  const result = await readLimitedJson<Record<string, unknown>>(
    response,
    MAX_RESPONSE_BYTES,
  )

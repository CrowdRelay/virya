...
const sendWelcome = async (data: Record<string, unknown>, eventId: string) => {
  ...
  await mailer.send({
    ...
    html: `<!doctype html><html><body style="margin:0;background:#09090b;color:#e4e4e7;font-family:Arial,sans-serif"><div style="max-width:600px;margin:auto;padding:40px 24px"><p style="color:#84b4ac;font-size:12px;font-weight:800;letter-spacing:.18em">${isPolish ? "VIRYA // SYGNAŁ" : "VIRYA // SIGNAL"}</p><h1 style="color:#fff">${isPolish ? "Sygnał aktywny" : "Signal active"}</h1><p style="line-height:1.7">${isPolish ? "Od teraz koncerty, Gra Virya, limitowane pule nagród i korzyści merchowe są połączone w jednym prostym systemie." : "Shows, AREA, limited reward pools and merch benefits can now work as one system."}</p><p><a href="${accountUrl}" style="display:inline-block;background:#84b4ac;color:#09090b;padding:16px 22px;text-decoration:none;font-weight:800">${isPolish ? "OTWÓRZ MÓJ SYGNAŁ" : "OPEN MY SIGNAL"}</a></p><p style="margin-top:28px;font-size:13px">${isPolish ? "Twój link polecający:" : "Your referral link:"}<br><a style="color:#84b4ac" href="${referralUrl}">${referralUrl}</a></p><p style="margin-top:36px"><a style="color:#71717a;font-size:11px" href="${unsubscribeUrl}">${isPolish ? "Wypisz mnie" : "Unsubscribe"}</a></p></div></body></html>`,
  })
}

...
const sendConfirmation = async (data: Record<string, unknown>) => {
  ...
  await mailer.transporter.sendMail({
    ...
    to: email,
    replyTo: mailer.to,
    subject: isPolish ? "Potwierdź swój Virya Signal" : "Confirm your Virya Signal",
    text: isPolish
      ? `${name ? `Cześć ${name}!\n\n` : "Cześć!\n\n"}Potwierdź adres, aby aktywować Virya Signal:\n${confirmationUrl}\n\nSignal łączy koncerty, AREA, nagrody i merch Viryi. Jeśli to nie Ty, zignoruj wiadomość.`
...
const sendWelcome = async (data: Record<string, unknown>) => {
  ...
  await mailer.transporter.sendMail({
    ...
    to: email,
    replyTo: mailer.to,
    subject: isPolish ? "Twój Virya Signal jest aktywny" : "Your Virya Signal is active",
    text: isPolish
      ? `Twój Signal działa.\n\nMój Signal: ${accountUrl}\nLink polecający: ${referralUrl}\n\nWypisz się: ${unsubscribeUrl}`
      : `Your Signal is active.\n\nMy Signal: ${accountUrl}\nReferral link: ${referralUrl}\n\nUnsubscribe: ${unsubscribeUrl}`,
    html: `<!doctype html><html><body style="margin:0;background:#09090b;color:#e4e4e7;font-family:Arial,sans-serif"><div style="max-width:600px;margin:auto;padding:40px 24px"><p style="color:#fbbf24;font-size:12px;font-weight:800;letter-spacing:.18em">VIRYA // SIGNAL</p><h1 style="color:#fff">${isPolish ? "Sygnał aktywny" : "Signal active"}</h1><p style="line-height:1.7">${isPolish ? "Od teraz koncerty, AREA, limitowane pule nagród i korzyści merchowe mogą pracować jako jeden system." : "Shows, AREA, limited reward pools and merch benefits can now work as one system."}</p><p><a href="${accountUrl}" style="display:inline-block;background:#fbbf24;color:#09090b;padding:16px 22px;text-decoration:none;font-weight:800">${isPolish ? "OTWÓRZ MÓJ SIGNAL" : "OPEN MY SIGNAL"}</a></p><p style="margin-top:28px;font-size:13px">${isPolish ? "Twój link polecający:" : "Your referral link:"}<br><a style="color:#fbbf24" href="${referralUrl}">${referralUrl}</a></p><p style="margin-top:36px"><a style="color:#71717a;font-size:11px" href="${unsubscribeUrl}">${isPolish ? "Wypisz mnie" : "Unsubscribe"}</a></p></div></body></html>`,
  })
}
